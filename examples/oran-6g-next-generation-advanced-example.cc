...
private:
    ...
    {
        uint32_t total = 0;
        for (const auto &[metric, values] : m_edgeAiMetrics)
            total += values.size();
        for (const auto &[metric, values] : m_metaverseMetrics)
            total += values.size();
        for (const auto &[metric, values] : m_securityMetrics)
            total += values.size();
        for (const auto &[metric, values] : m_performanceMetrics)
            total += values.size();
        return total;
    }
