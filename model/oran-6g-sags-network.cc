...
namespace ns3
{
                                                    ...
                                                    const std::vector<OrbitalParameters> &orbital_params)
    {
        ...
        NS_LOG_INFO("Deployed " << num_satellites << " satellites in " << static_cast<int>(type) << " constellation");
    }
    ...
    Oran6gSagsNetwork::DeployHapsNetwork(const std::vector<HapsNode> &haps_nodes)
    {
        ...
        for (size_t i = 0; i < haps_nodes.size(); ++i)
        {
            ...
            NS_LOG_INFO("Deploying HAPS node " << i << " at position (" << haps_nodes[i].position.x << ", " << haps_nodes[i].position.y << ", " << haps_nodes[i].position.z << ") km");

            // Validate altitude for HAPS deployment
            if (haps_nodes[i].position.z < 15.0 || haps_nodes[i].position.z > 50.0)
            {
                NS_LOG_WARN("HAPS node " << i << " deployed at non-optimal altitude: " << haps_nodes[i].position.z << " km");
            }
        }
    ...
    Oran6gSagsNetwork::ConfigureUnderwaterComm(const AcousticParameters &params)
    {
        ...
        {
            NS_LOG_WARN("Acoustic frequency " << params.frequency << " Hz may not be optimal for underwater communication");
        }
        ...
        {
            NS_LOG_WARN("Configured range " << params.max_range << " km exceeds theoretical maximum " << theoretical_max_range << " km");
        }

        NS_LOG_INFO("Configured underwater acoustic communication: " << "frequency=" << params.frequency << " Hz, " << "bandwidth=" << params.bandwidth << " Hz, " << "max_range=" << params.max_range << " km");
                                                   ...
                                                   const std::map<std::string, double> &handover_criteria)
    {
        ...
        NS_LOG_INFO("Optimized handover from domain " << static_cast<int>(source_domain) << " to domain " << static_cast<int>(target_domain) << ": latency=" << handover.handover_latency << " ms, " << "success_rate=" << handover.success_probability);
    }
    ...
    Oran6gSagsNetwork::OrchestrateSagsResources(const CoverageRequirement &requirement)
    {
        ...
        for (const auto &area : requirement.service_areas)
        {
            ...
            // Data rate check
            if (requirement.min_data_rate > 1000.0)
            { // Gbps
        ...
        NS_LOG_INFO("SAGS resource orchestration success rate: " << orchestration_success_rate * 100.0 << "%");
    ...
    Oran6gSagsNetwork::GenerateNetworkReport(const std::string &report_format) const
    {
        ...
        if (report_format == "JSON")
        {
            ...
            {
                report << "      \"domain_" << static_cast<int>(it->first) << "\": " << std::fixed << std::setprecision(2) << it->second;
                if (std::next(it) != m_coverage_statistics.end())
                    report << ",";
                report << "\n";
            }
        ...
        else
        {
            ...
            {
                report << "- " << metric.first << ": " << std::fixed << std::setprecision(2) << metric.second << "\n";
            }

            report << "\nCoverage Statistics:\n";
            for (const auto &coverage : m_coverage_statistics)
            {
                report << "- Domain " << static_cast<int>(coverage.first) << ": " << std::fixed << std::setprecision(1) << coverage.second << "%\n";
            }
        }
    ...
    Oran6gSagsNetwork::CalculateCoverageForArea(const Vector &area_center, double area_radius)
    {
        ...
        if (area_radius < 50.0)
        { // Within 50 km, terrestrial is strong
            total_coverage += 85.0;
